(() => {
  ...
  async function openDashboard(profile) {
    ...
    window.__appRole = profile.role;
    window.currentProfile = profile;
    try { await window.ensureRoleModules?.(profile.role); } catch (err) { console.warn('role modules', err); }
    if (typeof window.loadDashboard === 'function') {
      await window.loadDashboard(profile);
      window.dispatchEvent(new CustomEvent('dashboard:loaded',{detail:{profile}}));
      return true;
    }
  ...
  function ensureInstructions() {
    ...
    if (title) title.insertAdjacentElement('afterend', box); else card.prepend(box);
  }
  ...
  async function robustSignIn() {
    ...
    try {
      ...
      if (!profile) { await client.auth.signOut(); if (msg) msg.textContent = 'Esta cuenta no tiene acceso al simulador.'; return; }
  ...
  async function activateCandidateAccount() {
    ...
    if (!/^\S+@\S+\.\S+$/.test(email) || password.length < 8) { if (msg) msg.textContent = 'Ingresá el correo autorizado y una contraseña de al menos 8 caracteres.'; return; }
    if (!/^[A-F0-9]{8}$/.test(activationCode)) { if (msg) msg.textContent = 'Ingresá el código de activación de 8 caracteres entregado por el administrador.'; return; }
    if (btn) btn.disabled = true;
    ...
    try {
      const response = await fetch(`${window.APP_CONFIG.SUPABASE_URL}/functions/v1/candidate-activate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': window.APP_CONFIG.SUPABASE_KEY }, body: JSON.stringify({ email, password, activationCode })
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result?.ok) {
        const messages = {not_invited:'Este correo no fue habilitado por el administrador.',candidate_already_active:'Esta candidatura ya tiene una cuenta activa.',email_in_use:'Este correo ya está vinculado a otra cuenta. Usá otro correo.',invalid_credentials:'Revisá el correo, la contraseña y el código de activación.',invalid_code:'El código de activación no es correcto.',code_expired:'El código venció. Pedí al administrador que genere uno nuevo.',access_expired:'El período de acceso ya venció. Pedí al administrador que lo renueve.',too_many_attempts:'Se alcanzó el límite de intentos. Pedí un código nuevo.',candidate_inactive:'Esta candidatura no está habilitada.'};
      ...
      const profile = await getProfile();
      if (!profile || profile.access_active === false) { await client.auth.signOut(); if (msg) msg.textContent = 'La cuenta se activó, pero el período de acceso ya no está vigente.'; return; }
      await openDashboard(profile);
    } catch (error) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (signIn) { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); robustSignIn(); return; }
    const activate = event.target instanceof Element ? event.target.closest('#signUpBtn') : null;
    if (!activate) return;
    event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); activateCandidateAccount();
  }, true);

  function adjustCopy() {
    ensureActivationField(); ensureInstructions();
    const btn = document.getElementById('signUpBtn'); if (btn) btn.textContent = 'Activar cuenta';
    document.getElementById('noEmailVerificationHelp')?.remove();
  }

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', adjustCopy, {once:true}); else adjustCopy();
})();
